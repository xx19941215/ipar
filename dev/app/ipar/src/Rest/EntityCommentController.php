...
class EntityCommentController extends \Gap\Routing\Controller
{
    ...
    {
        $query = [
            'entity_type_id' => $this->request->request->get('dst_type'),
            'eid' => $this->request->request->get('dst_id'),
        ];
        $comment_set = $this->service('entity_comment')->search($query);
    ...
    public function createPost()
    {
        ...
        $data['entity_type_id'] = $this->request->request->get('dst_type_id');
        $data['eid'] = $this->request->request->get('dst_id');
        $data['content'] = $this->request->request->get('content');
        ...
        if ($reply_id && $reply_uid) {
            $data['opts']['reply_id'] = $reply_id;
            $data['opts']['reply_uid'] = $reply_uid;

            $reply_comment = $this->service('entity_comment')->findCommentById($reply_id);
            $data['opts']['conv'] = $reply_comment->conv;
        }

        $pack = $this->service('entity_comment')->createComment($data);
    ...
    public function deletePost()
    {
        ...
        $comment = $this->service('entity_comment')->findCommentById($comment_id);
